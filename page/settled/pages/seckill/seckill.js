...
/**
  ...
  onLoad: function (options) { 
    ...
    if (options.gid || 39068){
      this.setData({
        gid: options.gid || 39068,
        id: options.gid || 39068,
  ...
  getbrandDetail: function () {
    ...
    var exh = Dec.Aese('mod=show&operation=brandSummary&brandId=' + _this.data.brandId + '&page=0&uid=' + _this.data.uid + '&loginid=' + _this.data.loginid + '&type=drying&settlement=0');
      ...
      success: function (res) {
        ...
        if (res.data.ReturnCode == 200) {
          ...
          if(type == 0){
           ...
           if(zunmdata.isGoodsPraise){
              gpraise = parseInt(gpraise) - 1;
           }else{
              gpraise = parseInt(gpraise) + 1;
           }
           zunmdata.gpraise = gpraise;
           zunmdata.isGoodsPraise = !zunmdata.isGoodsPraise;
            _this.setData({
              zunmdata:zunmdata
            });
          }
              ...
              _this.subscrfun();
                ...
                if(_this.data.zunmdata.status==2){
                    if(_this.data.zunmdata.isDisplayDeposit){
                        if(_this.data.zunmdata.depositEndTime){
                            if(_this.data.nowTime < _this.data.zunmdata.depositEndTime){
                              _this.dsbffunblock()
                            }else{
                              if(specialGoods == 1){
                                _this.subscrfun();
                ...
                if(_this.data.zunmdata.status == 2){
                    if(_this.data.zunmdata.debuff==0){
                      _this.dsbffunblock()
                    }
                }
            }
